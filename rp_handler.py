...
def upload_to_s3(file_path: str, bucket_name: str, object_key_prefix: str = "") -> str:
    """
    Upload a file to a DigitalOcean Spaces bucket (S3-compatible).
    ...
    try:
        ...
        raise ClientError(
            f"Failed to upload to Spaces: {str(e)}", operation_name="upload_file")
...
def audio_inpainting(
    ...
    Returns:
        str: Path to the inpainted audio file.
    ...
    try:
        output_audio_path = inpainter.inpaint(inpaint_input)
...
def handler(event):
    ...
    Args:
        event (dict): Contains the input data with 'audio_url', 'transcript', 'api_key', 'bucket_name',
                      and optional 'object_key_prefix'.
